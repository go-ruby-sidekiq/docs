...
`go-ruby-sidekiq/sidekiq` reimplements Ruby's `sidekiq` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-sidekiq/sidekiq` directly, with no Ruby runtime;
...
Because `sidekiq` is pure Go with cgo disabled, it also compiles to **WebAssembly**
